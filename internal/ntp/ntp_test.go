...
const (
	defaultOnlineNTPHost = "0.beevik-ntp.pool.ntp.org"
	onlineTestsEnv       = "PKG_ONLINE_TESTS"
	refID                = 0xc0a80001
	timeFormat           = "Mon Jan _2 2006  15:04:05.00000000 (MST)"
)

var host = defaultOnlineNTPHost

// init 保留上游认证测试通过 NTP_HOST 指定本地服务的入口.
// 在线 smoke test 仍必须额外设置 PKG_ONLINE_TESTS=1 才会访问该地址.
func init() {
	if configuredHost := strings.TrimSpace(os.Getenv("NTP_HOST")); configuredHost != "" {
		host = configuredHost
	}
}

// requireOnlineNTPHost 只在显式开启在线测试时返回公网 NTP 地址.
// NTP_HOST 可覆盖默认服务器, 但不会绕过 PKG_ONLINE_TESTS 门禁.
func requireOnlineNTPHost(t *testing.T) string {
	t.Helper()
	if os.Getenv(onlineTestsEnv) != "1" {
		t.Skip("Set PKG_ONLINE_TESTS=1 to run public NTP smoke tests")
	}
	return host
}

// assertValid 要求响应完整通过协议校验, 不再忽略 timeout 或 kiss-of-death.
func assertValid(t *testing.T, r *Response) {
	t.Helper()
	if r == nil {
		t.Error("Response unexpectedly nil")
		return
	}
	if err := r.Validate(); err != nil {
		t.Errorf("Response unexpectedly invalid: %v", err)
	}
}

// assertInvalid 要求响应无法通过协议校验.
func assertInvalid(t *testing.T, r *Response) {
	t.Helper()
	err := r.Validate()
	if err == nil {
		t.Error("Response unexpectedly valid")
	}
}
...
func TestOnlineQuery(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("query public NTP server %s: %v", host, err)
	}
	assertValid(t, r)
	logResponse(t, r)
}
...
func TestOnlineTime(t *testing.T) {
	...
	tm, err := Time(host)
	if err != nil {
		t.Fatalf("query public NTP time from %s: %v", host, err)
	}
	now := time.Now()
...
// TestLocalQuery 使用真实本地 UDP 报文覆盖默认 Query 成功路径.
func TestLocalQuery(t *testing.T) {
	host, done := startLocalNTPServer(t, 0, true)
	r, err := QueryWithOptions(host, QueryOptions{Timeout: time.Second})
	awaitLocalNTPServer(t, done)
	if err != nil {
		t.Fatalf("query local NTP server: %v", err)
	}
	assertValid(t, r)
}
...
func TestLocalQueryTimeout(t *testing.T) {
	...
	if !errors.As(err, &timeoutErr) || !timeoutErr.Timeout() {
		t.Fatalf("query local NTP server returned non-timeout error: %v", err)
	}
}

// TestLocalTime 使用真实本地 UDP 报文覆盖校时成功路径.
func TestLocalTime(t *testing.T) {
	host, done := startLocalNTPServer(t, 0, true)
	tm, err := Time(host)
	awaitLocalNTPServer(t, done)
	if err != nil {
		t.Fatalf("query local NTP time: %v", err)
	}
	assert.True(t, time.Since(tm).Abs() < time.Second)
}

// TestLocalTimeFailure 验证无效本地响应会返回系统时间和校验错误.
func TestLocalTimeFailure(t *testing.T) {
	host, done := startLocalNTPServer(t, 0, false)
	startedAt := time.Now()
	local, err := Time(host)
	finishedAt := time.Now()
	awaitLocalNTPServer(t, done)
	assert.Equal(t, ErrInvalidStratum, err)
	assert.True(t, !local.Before(startedAt) && !local.After(finishedAt))
}
