...
func TestDesCBCEnStringHex(t *testing.T) {
	...
	assert.Equal(t, "96d0028878d58c89", actual)
...
func TestDesCBCEnPKCS7StringHex(t *testing.T) {
	...
	assert.Equal(t, "4431cc0267954866", actual)
...
func TestDesCBCEnStringB58(t *testing.T) {
	...
	assert.Equal(t, "SE56JGzQwap", actual)
...
func TestDesCBCEnPKCS7StringB58(t *testing.T) {
	...
	assert.Equal(t, "CQaC5oNT7z5", actual)
...
func TestDesCBCEnStringB64(t *testing.T) {
	...
	assert.Equal(t, "ltACiHjVjIk=", actual)
...
func TestDesCBCEnPKCS7StringB64(t *testing.T) {
	...
	assert.Equal(t, "RDHMAmeVSGY=", actual)
...
func TestDesCBCEncrypt(t *testing.T) {
	...
	assert.Equal(t, "Ko1p3p1f3/k=", utils.B64Encode(actual))

	actual = DesCBCEncrypt(false, []byte(("1")), []byte("abcdefgh"))
	assert.Equal(t, "BBV4AdvFQhg=", utils.B64Encode(actual))

	// 指定向量加密
	actual = DesCBCEncrypt(false, b, []byte(("12345678")), []byte("abcdefgh"))
	assert.Equal(t, "qGrKD8THQqjHNafLu/KDK6cqWoTKRxExsh4k8TQQp3m5/6K/HDHFyg==", utils.B64Encode(actual))
}
...
func TestDesCBCEncryptPKCS7(t *testing.T) {
	...
	assert.Equal(t, "qGrKD8THQqjHNafLu/KDK6cqWoTKRxExsh4k8TQQp3n1QubNNBPFLA==", utils.B64Encode(actual))
}

func TestDesCBCDecrypt(t *testing.T) {
	expected := tmpS

	assert.Equal(t, *new([]byte), DesCBCDecrypt(false, nil, nil))
	assert.Equal(t, *new([]byte), DesCBCDecrypt(false, []byte(("1")), nil))
	assert.Equal(t, *new([]byte), DesCBCDecrypt(false, []byte(("1")), []byte("2")))
...
func TestDesCBCDecryptPKCS7(t *testing.T) {
	...
	assert.Equal(t, *new([]byte), DesCBCDecrypt(true, nil, nil))
	assert.Equal(t, *new([]byte), DesCBCDecrypt(true, []byte(("1")), nil))
	assert.Equal(t, *new([]byte), DesCBCDecrypt(true, []byte(("1")), []byte("2")))
	assert.Equal(t, []byte(expected),
		DesCBCDecrypt(true,
