...
1. 默认只新增或修改 `*_test.go`、测试数据和测试门禁. 唯一预批准例外是修复 `config/common` 现有测试助手的自包含与状态恢复能力; 该例外不得进入正常应用启动路径.
...
使用当前 Git 已跟踪 Go 包执行 `go test -count=1 -cover` 的结果如下:
...
2. `test: 固定核心包与四项目验证来源`: 落地 clean worktree 和单项目临时 `go.work use + replace` 协议.
3. `test: 收敛核心包测试助手状态`.
